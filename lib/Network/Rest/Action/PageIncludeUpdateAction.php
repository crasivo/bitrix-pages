...
use Crasivo\Pages\Domain\Api\UpdatePageInclude;
use Crasivo\Pages\Network\Shared\Action\MacroConverterJson;
use Crasivo\Pages\Network\Shared\Filter\ModuleRightFilter;
use Crasivo\Pages\Network\Shared\Request\PageIncludeUpdateRequest;

/**
 * @final
 * @internal
 * @restUrl /rest/crasivo:pages.route.updateInclude
 */
final class PageIncludeUpdateAction extends Action
{
     ...
     * @param PageIncludeUpdateRequest $requestData
     * @return mixed
     * @throws \Throwable
     */
    public function run(
        PageRoute $pageRoute,
        PageIncludeUpdateRequest $requestData,
    )
    {
        // check content type
        if (!($pageRoute instanceof PageInclude)) {
        ...
        $pageRoute = ServiceLocator::getInstance()
            ->get(UpdatePageInclude::class)
            ->do(
                pageInclude: $pageRoute,
                rawPageInclude: new RawPageInclude(
                    isActive: $requestData->isActive,
                    routeDomain: $requestData->routeDomain,
                    routePath: $requestData->routePath,
                    includePath: $requestData->includePath,
                    pageProperties: null,
                ),
            );
