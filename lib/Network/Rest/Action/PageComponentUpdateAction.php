...
use Crasivo\Pages\Domain\Api\UpdatePageComponent;
 ...
 * @final
 * @internal
 * @restUrl /rest/crasivo:pages.route.updateComponent
 */
class PageComponentUpdateAction extends Action
{
     ...
     * @param PageComponentUpdateRequest $requestData
     * @return mixed
     * @throws \Throwable
     */
    public function run(
        PageRoute $pageRoute,
        PageComponentUpdateRequest $requestData,
    )
    {
        // check content type
        if (!($pageRoute instanceof PageComponent)) {
        ...
        $pageRoute = ServiceLocator::getInstance()
            ->get(UpdatePageComponent::class)
            ->do(
                pageComponent: $pageRoute,
                rawPageComponent: new RawPageComponent(
