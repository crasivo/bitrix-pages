 ...
 * @internal
 */
class CreatePageComponent implements \Crasivo\Pages\Domain\Api\CreatePageComponent
{
    ...
    ): PageComponent
    {
        try {
            // create empty model
            $pageRoute = $this->pageRouteFactory->createComponent();

            // fill entity fields
            $pageRoute->setActive($rawPageComponent->isActive);
            $pageRoute->setRouteDomain($rawPageComponent->routeDomain);
            $pageRoute->setRoutePath($rawPageComponent->routePath);
            $pageRoute->setComponentName($rawPageComponent->componentName);
            $pageRoute->setComponentParams($rawPageComponent->componentParams);
            $pageRoute->setComponentTemplate($rawPageComponent->componentTemplate);
